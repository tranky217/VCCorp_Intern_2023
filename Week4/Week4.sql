...
-- select more fields instead of select distinct
SELECT DISTINCT name FROM songs;
SELECT name, album_id FROM songs;

-- join with inner join insteads of using where
-- where will make all possible combination -> heavy operation
SELECT songs.id, songs.name, songs.album_id
FROM songs, albums
...
-- avoid using multiple or in filter
SELECT * FROM songs
WHERE name LIKE 'p%'
OR album_id = 1;
-- using union
